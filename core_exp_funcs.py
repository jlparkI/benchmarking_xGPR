...
from constants import protein_constants
...
def tune_model_bayes(train_dset, low_train_rffs, high_train_rffs,
    ...
    Args:
        ...
        pretransform_dir: The location for pretransformation (if desired)
            for fine-tuning.
        nmll_mode (str): The method for preconditioner construction for
            nmll evaluation.
...
def run_cv(start_dir, target, low_train_rffs, high_train_rffs,
        fitting_rff = 32768, precond_rank = 1024, kernel_choice = "Matern",
        random_seed = 123):
    """For some tabular datasets, we need to run a 5x CV
    since there is no pre-designated test set."""
    train_dset = get_train_dataset(start_dir, target, kernel_choice)

    xdata = [np.load(f) for f in train_dset.get_xfiles()]
    ydata = [np.load(f) for f in train_dset.get_yfiles()]
    xdata, ydata = np.vstack(xdata), np.concatenate(ydata)

    rng = np.random.default_rng(random_seed)
    idx = rng.permutation(train_dset.get_ndatapoints())
    idx = np.array_split(idx, 5)

    scores, tune_clocks, fit_clocks = [], [], []
    ...
    for i, test_idx in enumerate(idx):
        ...
        testx, testy = xdata[test_idx,:], ydata[test_idx]
        xmean, xstd = np.mean(trainx, axis=0), np.std(trainx, axis=0)
        trainx = (trainx - xmean[None,:]) / xstd[None,:]
        testx = (testx - xmean[None,:]) / xstd[None,:]

        train_dset = build_online_dataset(trainx, trainy)

        xgp, _, _, tuning_wclock, _ = tune_model_bayes(train_dset, low_train_rffs,
                                high_train_rffs, kernel_choice)
        xgp.fitting_rffs = fitting_rff
        score, _, fitting_wclock = online_fit_and_score(xgp, train_dset,
                            testx, testy, precond_max_rank = precond_rank)
        scores.append(score)
        tune_clocks.append(tuning_wclock)
        fit_clocks.append(fitting_wclock)

    score = np.mean(scores)
    tuning_wclock = np.mean(tune_clocks)
    fitting_wclock = np.mean(fit_clocks)
    score_std_dev = np.std(scores, ddof=1)
    print(f"CV scores: {scores}", flush=True)

    return score, tuning_wclock, fitting_wclock, score_std_dev
